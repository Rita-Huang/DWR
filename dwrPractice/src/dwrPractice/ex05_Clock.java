 ...
 */
public class ex05_Clock implements Runnable, UninitializingBean
{
    //本例用到的DWR API
    //執行緒池:   new ScheduledThreadPoolExecutor(int corePoolSize).scheduleAtFixedRate(Runnable command,long initialDelay,long period,TimeUnit unit);
    //Browser.withAllSessions(new Runnable(){public void run(){.....}}) 
    //呼叫JavaScript的方法:    ScriptSessions.addFunctionCall(java.lang.String funcName, java.lang.Object... params)
    //Browser.withAllSessionsFiltered(ScriptSessionFilter filter, Runnable task)
...
//        System.out.println("Clock建構子");
    ...
    public void destroy()
    {
        System.out.println("Clock  >  destroy()");
        executor.shutdown();
        try {
    ...
    {
        System.out.println("Clock  >  toggle()");
        active = !active;
        setClockStatus();
    }
    ...
    private class UpdatesEnabledFilter implements ScriptSessionFilter {
    	...
    	public UpdatesEnabledFilter(String attrName) {
    	    System.out.println("UpdatesEnabledFilter 建構子");
    		this.attrName = attrName;
    	}

		public boolean match(ScriptSession ss) {
		    System.out.println("UpdatesEnabledFilter  >  match   "  + new Date().toString());
			
		    //傳入attrName找有無ScriptSession 有吻合 有回傳true, 無則false
    ...
    public void setClockStatus()
    {
        Browser.withAllSessions(new Runnable()
        {
            public void run()
            {
//                System.out.println("setClockStatus-2  run()  "+new Date().toString());
    ...
    public void setEnabledAttribute(Boolean enabled) {
        System.out.println("setEnabledAttribute");
